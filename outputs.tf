output "proxy_ip" {
  description = "IP address of the runner proxy. Null when restrict_ingress is enabled."
  value       = var.restrict_ingress ? null : (var.loadbalancer_type == "external" ? google_compute_global_address.proxy_ip[0].address : google_compute_address.proxy_internal_ip[0].address)
}

output "load_balancer_backend_services" {
  description = "Name of the proxy backend service. Null when restrict_ingress is enabled."
  value       = var.restrict_ingress ? null : (var.loadbalancer_type == "external" ? google_compute_backend_service.proxy[0].name : google_compute_region_backend_service.proxy_internal[0].name)
}

output "load_balancer_ip" {
  description = "IP address of the runner load balancer. Null when restrict_ingress is enabled."
  value       = var.restrict_ingress ? null : (var.loadbalancer_type == "external" ? google_compute_global_address.proxy_ip[0].address : google_compute_address.proxy_internal_ip[0].address)
}
...
output "proxy_instance_group_name" {
  description = "Name of the proxy VM instance group manager. Null when restrict_ingress is enabled."
  value       = var.restrict_ingress ? null : google_compute_region_instance_group_manager.proxy[0].name
}
