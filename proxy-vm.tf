...
data "cloudinit_config" "proxy_additional" {
  for_each = local.additional_proxy_regions
...
resource "google_compute_instance_template" "proxy_additional" {
  for_each = local.additional_proxy_regions

  name_prefix  = "${var.runner_name}-${each.key}-proxy-"
  project      = var.project_id
  machine_type = var.proxy_vm_config.machine_type
  region       = each.value.region
  ...
  labels = merge(local.proxy_labels, {
    region = each.value.region
  })
  ...
  disk {
    ...
    dynamic "disk_encryption_key" {
      ...
      content {
        kms_key_self_link = each.value.kms_key_name
      }
    }
  }
  ...
  metadata = {
    ...
    user-data = sensitive(data.cloudinit_config.proxy_additional[each.key].rendered)
...
resource "google_compute_region_instance_group_manager" "proxy_additional" {
  for_each = local.additional_proxy_regions

  name                      = "${var.runner_name}-${each.key}-proxy-group"
  region                    = each.value.region
  project                   = var.project_id
  base_instance_name        = "${var.runner_name}-${each.key}-proxy"
  distribution_policy_zones = each.value.zones

  version {
    instance_template = google_compute_instance_template.proxy_additional[each.key].id
  }
  ...
  update_policy {
    ...
    max_surge_fixed       = max(length(each.value.zones), 2)
    max_unavailable_fixed = var.proxy_vm_config.update_policy_config.max_unavailable == 0 ? 0 : max(length(each.value.zones), var.proxy_vm_config.update_policy_config.max_unavailable)
...
resource "google_compute_region_autoscaler" "proxy_additional" {
  for_each = local.additional_proxy_regions

  name    = "${var.runner_name}-${each.key}-proxy-autoscaler"
  region  = each.value.region
  target  = google_compute_region_instance_group_manager.proxy_additional[each.key].id
  project = var.project_id
